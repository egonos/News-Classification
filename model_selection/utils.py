...
def load_amazon_data():
  ...
  df = pd.concat([negative_samples,positive_samples],ignore_index=True).sample(frac=1).reset_index(drop = True)
  return df

def load_reviews_data():
  data_path = "/root/.cache/kagglehub/datasets/ahmedabdulhamid/reviews-dataset/versions/1"
  ...
  df = pd.concat([df_pos, df_neg], ignore_index=True).sample(frac = 1,random_state=42,replace=False).reset_index(drop = True)
  return df
...
def run_model(MODEL_NAME, MAX_LENGTH, LABEL_MAP, TEST_DF, DF_NAME):
  ...
  logits = [log["score"] if (log["label"] == "POSITIVE") or (log["label"] == "LABEL_1") else 1 - log["score"] for log in logs]
